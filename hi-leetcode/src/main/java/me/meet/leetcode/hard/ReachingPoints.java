...
public final class ReachingPoints {
    private ReachingPoints() {
    }
    /**
    ...
    // better, faster
    static boolean reachingPoints2(int sx, int sy, int tx, int ty) {
        while (tx >= sx && ty >= sy) {
            if (tx > ty) {
                tx %= ty;
            } else {
    ...
    private static void testReachingPoints() {
        int sx = 1, sy = 1, tx = 3, ty = 5;
        boolean res = reachingPoints(sx, sy, tx, ty);
        boolean res2 = reachingPoints2(sx, sy, tx, ty);
